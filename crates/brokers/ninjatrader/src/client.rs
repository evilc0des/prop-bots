...
/// Configuration for connecting to NinjaTrader 8.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NinjaTraderConfig {
    /// Host address (e.g. "127.0.0.1").
    pub host: String,
    /// Port the NT8 addon is listening on.
    pub port: u16,
...
/// NinjaTrader 8 broker adapter.
///
/// Communicates with a C# addon running inside NT8 via TCP socket
/// using length-prefixed JSON messages.
pub struct NinjaTraderBroker {
...
impl NinjaTraderBroker {
    pub fn new(config: NinjaTraderConfig) -> Self {
        Self {
            config,
            stream: None,
            connected: false,
    ...
    /// Read a single framed message from NinjaTrader.
...
#[async_trait]
impl Broker for NinjaTraderBroker {
    async fn connect(&mut self) -> Result<(), BrokerError> {
        let addr = format!("{}:{}", self.config.host, self.config.port);
        info!("Connecting to NinjaTrader at {}", addr);
        ...
        match msg {
            InboundMessage::Connected { version } => {
                info!("Connected to NinjaTrader addon v{}", version);
                self.connected = true;
            }
    ...
    async fn disconnect(&mut self) -> Result<(), BrokerError> {
        ...
        info!("Disconnected from NinjaTrader");
        Ok(())
    }
    ...
    async fn subscribe_market_data(
        ...
        // TODO: Spawn a task that reads from the TCP stream and forwards to the channel
